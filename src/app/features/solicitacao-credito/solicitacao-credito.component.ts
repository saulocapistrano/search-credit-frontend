...
import { SolicitacaoCreditoService } from '../../core/services/solicitacao-credito.service';
...
@Component({
  ...
  styleUrl: './solicitacao-credito.component.scss'
})
export class SolicitacaoCreditoComponent implements OnInit {
  private readonly formBuilder = inject(FormBuilder);
  private readonly solicitacaoService = inject(SolicitacaoCreditoService);
  ...
  ngOnInit(): void {
    ...
    // Criar formulário
    this.solicitacaoForm = this.criarFormulario();
  ...
  private carregarDadosIniciais(): void {
    ...
    forkJoin({
      numeroCredito: this.solicitacaoService.getNextNumeroCredito(),
      numeroNfse: this.solicitacaoService.getNextNumeroNfse()
    }).subscribe({
      next: (response) => {
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (!ALLOWED_FILE_TYPES.includes(file.type)) {
        ...
        this.fileName = '';
        this.solicitacaoForm.patchValue({ comprovanteRenda: null });
        input.value = '';
        return;
      }
      ...
      this.solicitacaoForm.patchValue({ comprovanteRenda: file });
    }
  }

  onSubmit(): void {
    if (this.solicitacaoForm.invalid || !this.comprovanteFile) {
      this.markFormGroupTouched(this.solicitacaoForm);
    ...
    this.solicitacaoService.criarCredito(dados, this.comprovanteFile).subscribe({
      next: (response) => {
        this.isLoading = false;
        localStorage.setItem('nome-solicitante', dados.nomeSolicitante);
        this.toastService.success('Solicitação de crédito enviada com sucesso! Status: EM_ANALISE');
        this.resetForm();
      },
      error: (error) => {
        this.isLoading = false;
        this.toastService.error(error.message || 'Erro ao enviar solicitação. Tente novamente.');
      }
    });
  }
  ...
  isFieldInvalid(fieldName: string): boolean {
    const field = this.solicitacaoForm.get(fieldName);
